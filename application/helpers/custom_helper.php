...
/*
 * do uploding of Video  file
 * return destination file name
 */

function upload_orginal_video($param = array()) {
    $ci = & get_instance();
    $ci->load->library('my_upload');
    if ($param["file"]["name"] != "") {
        $ci->my_upload->upload($param["file"]);
...
function is_selected_page($current, $loaded) {
    return ($current == $loaded) ? 'class="select"' : '';
}
...
function unlink_image($name, $ext) {
    $ci = & get_instance();
    @unlink($ci->config->item('image_path') . $name . '_o.' . $ext);
    @unlink($ci->config->item('image_path') . $name . '_s.' . $ext);
    @unlink($ci->config->item('image_path') . $name . '_m.' . $ext);
    @unlink($ci->config->item('image_path') . $name . '_l.' . $ext);
}
...
function select_field($table, $field, $id, $parameter) 
{
	$ci =& get_instance();
	$ci->db->select($parameter); 
...
function message_error($msgtype, $description) {
    if ($msgtype != "" && $description != "") {
        return '<script type="text/javascript">
                        $(document).ready(function() {
                            if( $.fn.jGrowl ) {
...
function message_success($msgtype, $description) {
    if ($msgtype != "" && $description != "") {
        return '<script type="text/javascript">
...
function getPagination($recCount, $no_of_paginations, $cur_page, $first_btn, $last_btn, $previous_btn, $next_btn) {
    ...
    if ($first_btn && $cur_page > 1) {
        $html .= '<li p="1" class="first prev clickable"><a class="pagination_link">First</a></li>';
    } else if ($first_btn) {
    ...
    if ($previous_btn && $cur_page > 1) {
        $pre = $cur_page - 1;
        $html .= '<li p=' . $pre . ' class="prev clickable"><a class="pagination_link">Previous</a></li>';
    ...
    for ($i = $start_loop; $i <= $end_loop; $i++) {
        if ($cur_page == $i) {
            ...
            $html .= '<li p=' . $i . ' class="clickable" ><a class="pagination_link">' . $i . '</a></li>';
        }
    }
    ...
    if ($next_btn && $cur_page < $no_of_paginations) {
        $nex = $cur_page + 1;
        $html .= '<li p=' . $nex . ' class="next clickable"><a class="pagination_link">Next</a></li>';
    } else if ($next_btn) {
    ...
    if ($last_btn && $cur_page < $no_of_paginations) {
        $html .= '<li p=' . $no_of_paginations . ' class="last next clickable"><a class="pagination_link">Last</a></li>';
    } else if ($last_btn) {
