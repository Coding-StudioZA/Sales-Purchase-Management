...
class Install extends CI_Controller {
	...
	function do_install()
	{
		...
		if($db_verify == true)
		{
			...
			$data = read_file(FCPATH.'/application/config/database.php');
			...
			$data2 = read_file(FCPATH.'/application/config/routes.php');
			$data2 = str_replace('install','dashboard',$data2);
			write_file(FCPATH.'/application/config/routes.php', $data2);
			
			
			// Run the installer sql schema		
			$this->load->database();
			
			$schema = read_file(FCPATH.'/assets/db/db.sql');
			...
			$this->db->where('id' , 1);
			$this->db->update('administrator' , array('username'	=>	$this->input->post('email'),
